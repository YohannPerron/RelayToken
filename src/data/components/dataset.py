...
from src.data.components.writer import ArchaeoWindowWriter, EmptyWindowWriter
from src.utils.data import apply_mapping
...
class Sampler:
    ...
    def sample_multi_scale(
        ...
        for i in range(len(self.factors_m) - 1, -1, -1):
            ...
            scaling_factor = int(px_scaling_factor / m_scaling_factor)
            ...
            WH = int(side_px[0] * px_scaling_factor), int(
                side_px[1] * px_scaling_factor
            )
            (
                I_nrm,
    ...
    def sample_single_scale(
        ...
        """Sample data from the source at a single scale. Needs to be implemented by the child
        class.
...
class TiffSampler(Sampler):
    def __init__(
        ...
        super().__init__(aoi_name, factors_m, factors_px, mask_geometry=mask_geometry)
        self.raster_path = raster_path
        self.iinter = iinter
        if dowsample_path is not None:
            self.dowsample_paths = {}
            # glob all fill paths {aoi_name}_downsample_x{downsample_factor}.tif
            downsamples_files = glob.glob(
                str(dowsample_path) + f"/{aoi_name}_downsample_x*.tif"
            )
            # extract downsample factors
            for file in downsamples_files:
                factor = int(file.split("_")[-1].split(".")[0][1:])
                self.dowsample_paths[factor] = file

            # sort the factors
            self.dowsample_paths = dict(sorted(self.dowsample_paths.items()))
        else:
            self.dowsample_paths = None
    ...
    def sample_single_scale(
        ...
        if self.raster_path is None:
            return (
                np.zeros(side_px, dtype=self.out_type),
                safft_world_to_icell,
                safft_icell_to_tcell,
                None,
            )

        if scaling is not None and self.dowsample_paths is not None:
            # look for the tiff with the right scaling
            for file_scaling, path in self.dowsample_paths.items():
                if file_scaling <= scaling:
                    raster_path = path
                    break
        ...
        if self.mask_geometry is not None:
            ...
            tcell["img"].mask = np.logical_or(tcell["img"].mask, area_mask[..., None])

        if self.out_type is not None:
            tcell["img"] = tcell["img"].astype(self.out_type)
        return (
            tcell["img"],
            icell["safft_world_to_icell"],
            tcell["safft_icell_to_tcell"],
...
class TDataset_gtiff(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            label_map (Dict[int, int], optional): Mapping of labels. Defaults to None.
    ...
    def sample_inputs(self, geometry, side_px):
        ...
        for sampler in self.inputSamplers:
            ...
            (
                ...
                safft_icell_to_tcell,
                new_window,
            ) = sampler.sample_multi_scale(geometry, side_px, safft_world_to_icell)
            images_list.append([])
            ...
            if new_window is not None:
                assert window is None or window == new_window, "Window conflict"
                window = new_window
        ...
        for scale in range(len(images_list[0])):
            ...
            images_scale = [images_list[k][scale] for k in range(len(images_list))]
            image = np.ma.concatenate(images_scale, axis=-1)
            mask = image.mask
            ...
            if isinstance(mean, List):
                mean = [
                    e.filled(0) if isinstance(e, np.ma.MaskedArray) else e for e in mean
                ]
            mean = np.asarray(mean)
            img_nrm = (image.filled(mean) - mean) / self.std

            I_nrm = torch.as_tensor(img_nrm).to(torch.float32)
            assert I_nrm.ndim != 2, f"Image has to be 3D, current shape{I_nrm.shape}"
    ...
    def sample_target(self, geometry, safft_world_to_icell, safft_icell_to_tcell, WH):
        ...
        for sampler in self.targetSamplers:
            ...
            (
                ...
                safft_icell_to_tcell,
                window,
            ) = sampler.sample(geometry, WH, safft_world_to_icell, safft_icell_to_tcell)
            if target.ndim == 2:
                target = np.expand_dims(target, axis=-1)
            targets.append(target)
        # concatenate all targets
        target = np.ma.concatenate(targets, axis=-1)
    ...
    def __getitem__(self, index):
        ...
        if self.file_writer is not None:
            window_writer = ArchaeoWindowWriter(
                self.file_writer,
                window,
                square=row_square["geometry"],
                safft_world_to_icell=safft_world_to_icell,
                safft_icell_to_tcell=safft_icell_to_tcell,
                poly_aoi=self.inputSamplers[0].mask_geometry,
            )
        else:
            window_writer = EmptyWindowWriter()
        meta = {
            "index": index,
            "aoi_name": self.aoi_name,
            "mask": mask,
            "window_writer": window_writer,
        }
        ...
        if self.generate_targets:
            ...
            if targets.dtype == torch.float32 or targets.dtype == torch.float64:
                ...
                ), "Target map not supported for float targets"
            elif targets.dtype in [
                torch.int64,
                torch.int32,
                torch.uint8,
            ]:
                # apply self.label_map
                if self.label_map is not None:
                    targets = apply_mapping(targets, self.label_map)

                targets[mask.expand_as(targets)] = 255
            else:
                raise ValueError(f"Target dtype not supported: {targets.dtype}")
        else:
