...
def infer_output(
    model, num_channels, img_size, temporal_dim=0, indices=[], **kwargs
):
    ...
    Args:
        ...
        temporal_dim (int, optional): Number of frames. Defaults to 0.
        indices (list, optional): The indices of the output features to concider. Defaults to [].
    ...
    if len(indices) == 0:
        ...
        indices = range(len(dumy_features))
    if isinstance(dumy_features, (list, tuple)):
        dumy_features = [dumy_features[idx] for idx in indices]
        print(len(dumy_features))
        if len(dumy_features) == 1:
            dumy_features = dumy_features[0]
...
def get_first_layer(model):
    for module in model.modules():
        if isinstance(module, nn.Conv2d) and module.in_channels == 3:
            return module
        if isinstance(module, nn.Linear) and module.in_features == 3:
            return module
    return None
...
def set_first_layer(model, n_channels, is_rgb=None):
    ...
    module = get_first_layer(model)
    if module is None:
        raise ValueError("Could not find a suitable first layer")
