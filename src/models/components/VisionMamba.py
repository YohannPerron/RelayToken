# Modify from https://github.com/hustvl/Vim/blob/main/vim/models_mamba.py
#
########################################################################
...
from src.models.components.utils.rope import *
...
class VisionMamba(nn.Module):
    ...
    def forward_features(
        ...
        if if_random_token_rank:
            ...
            if isinstance(token_position, list):
                ...
                token_position = torch.where(
                    shuffle_indices == token_position
                )[0].item()
        ...
        if not self.fused_add_norm:
            ...
            fused_add_norm_fn = (
                rms_norm_fn
                if isinstance(self.norm_f, RMSNorm)
                else layer_norm_fn
            )
        ...
        # return averything except cls token
        if self.if_cls_token:
            if self.use_double_cls_token:
                return torch.cat(
                    (
                        ...
                        hidden_states[
                            :, token_position[0] + 1 : token_position[1], :
                        ],
                        hidden_states[:, token_position[1] + 1 :, :],
                    ),
                    dim=1,
                )
            else:
...
def checkpoint_filter_fn(state_dict, model):
    """Filter checkpoint to remove unnecessary keys."""
    state_dict = state_dict.get("model", state_dict)
    state_dict = state_dict.get("state_dict", state_dict)
    return state_dict


class ViM_module(nn.Module):
    def __init__(
        self,
        num_classes=4,
        num_channels=1,
        ...
        if pretrained:
            ...
            self.model.load_state_dict(
                checkpoint_filter_fn(checkpoint, self.model), strict=True
            )
            self.model.final_pool_type = "all"
        ...
        if lora_rank > 0:
            ...
            # unfreeze first layer to addapte to new modality
        ...
        self.feature_size = img_size // stride
        self.features_format = "NLC"
