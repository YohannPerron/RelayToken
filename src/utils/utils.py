...
def printlog_config_tree(cfg: DictConfig, label: str = "final") -> None:
    """
    Simplified, no-bs "rich_utils.print_config_tree"

    - Properly logs config, simple yaml, no pretty stuff
    """
    priority = (
        "data",
        "model",
    ...
    pcfg = DictConfig({f: cfg[f] for f in priority if f in cfg})
    pcfg.update({f: cfg[f] for f in cfg if f not in pcfg})
    log.info("Config ({}):\n{}".format(label, OC.to_yaml(pcfg, resolve=True)))
