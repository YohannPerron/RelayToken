...
class AoiSampler:
    ...
    def polygon_to_graph(self, polygon: LinearRing) -> Tuple[np.ndarray, np.ndarray]:
...
def sample_grid_squares_from_aoi_v2(
    ...
    gf_squares = (
        pd.concat(
            [
                gf_squares.sjoin(gf_poly, how="inner", predicate="within"),
                gf_squares.sjoin(gf_poly, how="inner", predicate="overlaps"),
            ]
        )
        .drop_duplicates()
    ...
    if not gf_squares.unary_union.contains(poly):
        missing_polygon = poly.difference(gf_squares.unary_union)
        missing_area = missing_polygon.area
        if missing_area > 1e-4:
            log.warning(
                f"Union of squares does not contain the target polygon (Missing area: {missing_area:.6f})"
            )
...
def rotate_shapes(rgen, shapes, angle, angle_prob):
    ...
    if angle_prob < 1.0:
        ...
        do_not_rotate = rgen.binomial(1, 1 - angle_prob, size=len(angles)).astype(bool)
        angles[do_not_rotate] = 0
    rot_shapes = [
        shapely.affinity.rotate(shape, angle) for shape, angle in zip(shapes, angles)
    ]
    return rot_shapes
...
def sample_squares_inside_aoi_leqn(
    ...
    squares = [box for box in squares if any((box.within(aoi) for aoi in aoi_polygons))]
    return squares
...
def compute_tcell_safft(
    ...
    """
    Takes bbox points from normalized bbox, starting from upper left, in a ccw order (inverted y). Produces
    ...
    Returns:
        Tuple: [
            ...
            width, height]
    """
    vec_x = SQR_icell[3] - SQR_icell[0]
    vec_y = SQR_icell[1] - SQR_icell[0]
    # Compute rotation angle
    angle_at2 = np.arctan2(-vec_x[1], vec_x[0])  # Y-coord is inverted, so -
    cosp = math.cos(angle_at2)
    sinp = math.sin(angle_at2)
    # Compute scaling factor
    vec_width = np.linalg.norm(vec_x)
    vec_height = np.linalg.norm(vec_y)
    if WH is None:
    ...
    # Compute translation (the point at 0 should be at [0, 0])
    x2, y2 = SQR_icell[0]
    x_off = -(a * x2 + b * y2)
    y_off = -(d * x2 + e * y2)
...
def compute_tcell_affine_transform_ocv(SQR_icell, WH=None):
    ...
    if WH is None:
        vec_width = np.linalg.norm(SQR_icell[3] - SQR_icell[0])
        vec_height = np.linalg.norm(SQR_icell[1] - SQR_icell[0])
        width, height = vec_width, vec_height
    else:
        width, height = WH
    wmat = cv2.getAffineTransform(
        SQR_icell[[3, 0, 1]].astype(np.float32),
...
def rasterio_urcrop(rasterpath, shapes):
    ...
    with rasterio.open(rasterpath) as src:
        ...
        window = rasterio.features.geometry_window(src, shapes, pad_x=0, pad_y=0)
...
def sample_icell_from_raster_v2(
    ...
    square_icell = affine_transform(square, safft_world_to_icell).normalize()
    return Icell(
        img=img_icell,
        square=square_icell,
...
def convert_icell_to_tcell_v2(
    ...
    safft_icell_to_tcell, tcell_height, tcell_width = compute_tcell_safft(SQR_icell, WH)
...
def convert_vicell_to_vtcell(
    ...
    # Affine transform features and square
    gf_cfeats_vtcell = gpd_affine_transform(gf_cfeats_vicell, safft_icell_to_tcell)
