...
def apply_mapping(
    ...
    input_tensor = isinstance(img, torch.Tensor)
    img_mapped = img.clone() if input_tensor else img.copy()
    if "cutoffid" in mapping:
        cutoffid = mapping.pop("cutoffid")
        cutoffto = mapping.pop("cutoffto")
        img_mapped[img >= cutoffid] = cutoffto
